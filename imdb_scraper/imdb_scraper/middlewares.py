    ...
    def process_response(self, request, response, spider):
        if isinstance(response, HtmlResponse):
            try:
                ...
                # Parse the response bytes as XML/HTML
                parser = etree.XMLParser(encoding='utf-8')
                xml = etree.parse(source=io.BytesIO(response_bytes), parser=parser)

                # Return a new response with the parsed body
                return response.replace(body=etree.tostring(xml, encoding='utf-8'))

            except Exception as e:
                spider.logger.error(f"Failed to force UTF-8 encoding and parse XML/HTML: {e}")
        return response
